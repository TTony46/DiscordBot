...
namespace PomodoroBot.Commands
{
    ...
    public class TimerCommands : BaseCommandModule
    {
        ...
        public async Task Join(CommandContext ctx, DiscordChannel channel = null)
        {
            ...
            await ctx.Channel.SendMessageAsync($"Attempting to connect to {channel.Name}");

            // connect
            vnc = await vnext.ConnectAsync(channel);
            await ctx.Channel.SendMessageAsync($"Connected to `{channel.Name}`");
        }
        ...
        public async Task Leave(CommandContext ctx)
        {
            ...
            {
                // not connected
                await ctx.Channel.SendMessageAsync("Not connected in this guild.");
                return;
            }
